...
int	main(void)
{
	...
	try {
		Bureaucrat tooHigh("MxelaShat", 0);
	}
	catch (std::exception &e) {
		std::cout << "Test 1 result: " << e.what() << std::endl;
	}

	try {
		Bureaucrat tooLow("ShatXreg", 151);
	}
	catch (std::exception &e) {
		std::cout << "Test 2 result: " << e.what() << std::endl;
	}

	try {
		Bureaucrat b("Sxaly", 1);
		b.incrementGrade(); 
	}
	catch (std::exception &e) {
		std::cout << "Test 3 result: " << e.what() << std::endl << std::endl;
	}
